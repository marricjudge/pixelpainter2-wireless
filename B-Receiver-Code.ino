/* ELEMENTS OF THE FUTURE
   ...
   Inspired by the Adafruit NeoPixel Painter tutorial https://learn.adafruit.com/neopixel-painter

   Code Part B: Receiver station
...
#define CURRENT_MAX 5500 // Max current from power supply (mA)
#define statusLED     3 // External LED on the Arduino
const int HC12TXpin     =  8;
const int HC12RXpin     =  9;
...
byte incomingByte;
String readBuffer = "";
int receivedSpeed = 100;
int receivedBrightness = 300;
  ...
  if (startupTrigger == HIGH) { // No button press
    ...
    for (int j = 0; i < 40; j++) {
      while (HC12.available()) { // If HC-12 has data
        ...
        receivedBrightness = readBuffer.toInt();
        receivedBrightness = (receivedBrightness * 2) / 2;

        if (receivedBrightness == 0) {
          receivedBrightness = 10;
          //Serial.print("Received brightness was 0 and set to 10");
        }

        Serial.print("receivedBrightness is ");
        Serial.println(receivedBrightness);

        //Serial.print("New minBrightness: ");
        //Serial.println(minBrightness);
      }
      break;
    }
    //receivedBrightness = 300;
    minBrightness = map(receivedBrightness, 0, 1023, 100, minBrightness);
...
void loop() {
  ...
  if (HC12.available()) { // If HC-12 has data
    ...
    receivedSpeed = readBuffer.toInt();
    receivedSpeed = (receivedSpeed * 2) / 2;
  ...
  if (incomingByte == 65) {
    ...
    uint32_t linesPerSec = map(receivedSpeed, 0, 1023, 10, maxLPS);
    Serial.print("Used received speed: ");
    ...
    for (;;) {
      ...
      if (++block >= nBlocks) {                  // Past last block?
        ...
        block = 0;      // Else trigger still held
      }
    ...
    HC12.write("Conf");      // Send that data to HC-12
    //Serial.println("Command confirm send");
    ...
    //if (++frame >= nFrames) frame = 0; //original part
